...
public class BluetoothLeService extends Service {
    ...
    public final static String ACTION_GET_DISTANCE =
                ...
                TimerTask tt = new TimerTask() {
                    ...
                    public void run() {
                        ...
                        /*runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(getApplicationContext(), "Distance = " + calculateAccurancy(rssiVal, txPower), Toast.LENGTH_SHORT).show();
                            }
                        });*/
                        //broadcastUpdate(ACTION_GET_DISTANCE);
                        Intent intent = new Intent(ACTION_GET_DISTANCE);
                        double acc = calculateAccurancy(rssiVal, txPower);
                        intent.putExtra("distance", acc);
                        Log.e(TAG, "---------------- rssi: " + rssiVal + ", acc: " + acc);
                        sendBroadcast(intent);
                    }
                };
                rssiTimer = new Timer();
                rssiTimer.schedule(tt, 2000, 2000);
...
//		            			System.out.println(mCharacteristicRingBuffer.size());
    ...
    public double calculateAccurancy(int rssi, int aTxPower){

        if (rssi == 0) {
            return -1.0; // if we cannot determine accuracy, return -1.
        }

        double ratio = rssi*1.0/aTxPower;
        if (ratio < 1.0) {
